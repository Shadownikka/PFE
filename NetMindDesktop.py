#!/usr/bin/env python3
"""
NetMind Desktop — Fully Native PyQt6 Application
No web server. No browser. Pure Qt widgets driving the network modules directly.

Run: sudo python3 NetMindDesktop.py
"""
...
from PyQt6.QtWidgets import (
    ...
    QSplashScreen, QAbstractItemView,
)
...
class InitWorker(QThread):
    ...
    def run(self):
        try:
            ...
            from tool import (get_gateway_ip, get_default_interface,
                              get_subnet_cidr, discover_clients,
                              TrafficMonitor, BandwidthController,
                              ConnectionTracker, enable_ip_forwarding)
            iface   = get_default_interface()
            gw_ip   = get_gateway_ip()
            my_ip   = scapy.get_if_addr(iface)
            gw_mac  = (scapy.getmacbyip(gw_ip) or "").lower()
            my_mac  = scapy.get_if_hwaddr(iface).lower()
            clients = discover_clients(get_subnet_cidr(iface))
            devices = {}
            for c in clients:
                ip = c["ip"]; mac = (c.get("mac") or "").lower()
                if ip in (gw_ip, my_ip) or mac in (gw_mac, my_mac):
                    continue
                devices[ip] = {"mac": mac, "name": f"Device-{ip.split('.')[-1]}"}
            monitor    = TrafficMonitor(devices)
            tracker    = ConnectionTracker(devices, iface)
            controller = BandwidthController(iface, monitor)
            controller.set_gateway({"ip": gw_ip, "mac": gw_mac})
...
class DiscoveryWorker(QThread):
    ...
    def run(self):
        try:
            from tool import get_subnet_cidr, discover_clients
            cidr    = get_subnet_cidr(self.iface)
            clients = discover_clients(cidr)
            found   = {}
            for c in clients:
                ip  = c["ip"]; mac = (c.get("mac") or "").lower()
                if ip in (self.gw_ip, self.my_ip): continue
                if mac in (self.gw_mac, self.my_mac): continue
                if ip not in self.known_ips:
                    found[ip] = {"mac": mac, "name": f"Device-{ip.split('.')[-1]}"}
            if found:
...
class DashboardPage(QWidget):
    ...
    def _build(self):
        ...
        for b in (self.btn_init, self.btn_start, self.btn_stop, self.btn_graf, self.btn_recfg):
            hl.addWidget(b)
        root.addWidget(hdr)
        # ── Body ────────────────────────────────────────────────
    ...
    def _on_init(self,d):
        ...
        try:
            from net_agent import NetMindAgent; from tool import Config
    ...
    def _start(self):
        ...
        import subprocess
        # ── Step 1: Flush ALL stale TC and iptables rules from previous sessions ──
        ...
        # Reset controller TC state to match (clean slate)
        self.controller._tc_active = False
        self.controller.limits.clear()
        # ── Step 2: Start monitors ────────────────────────────────────────────────
        self.monitor.start(); self.tracker.start()
        self.controller.set_gateway({"ip": self.gw_ip, "mac": self.gw_mac})
        # ── Step 3: Spoof all non-trusted devices ─────────────────────────────────
    ...
    def _stop(self):
        ...
        from tool import disable_ip_forwarding; disable_ip_forwarding()
    ...
    def _on_new_devices(self, found: dict):
        ...
        for ip, info in found.items():
            ...
            if ip not in self.trusted_ips and self.controller:
                self.controller.start_spoofing({"ip": ip, "mac": info.get("mac","")})
            # Allow iptables to track this device
            import subprocess
            subprocess.run(f"iptables -A FORWARD -s {ip} -j ACCEPT",
                           shell=True, stderr=subprocess.DEVNULL)
            subprocess.run(f"iptables -A FORWARD -d {ip} -j ACCEPT",
                           shell=True, stderr=subprocess.DEVNULL)
        self._sv(self.s_dev, len(self.devices))
    ...
    def _refresh_autopilot(self):
        ...
        from autopilot import load_decision_log
